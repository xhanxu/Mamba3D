...
def plot_tsne(
    ...
    plot_params = {
        ...
        # "edgecolors": kwargs.get("ec", 'black'),
    }
    ...
    # plt.scatter(x[:, 0], x[:, 1], c=point_colors, rasterized=True, **plot_params)
    # plt.xlim(-60, 60)
    # plt.ylim(-60, 60)

    plt.scatter(x[:, 0], x[:, 1], c=y, cmap='Spectral', **plot_params)
    # plt.scatter(x[:, 0], x[:, 1], c=y, cmap='Spectral', **plot_params)

    # Hide ticks and axis
    # plt.set_xticks([]), plt.set_yticks([]), plt.axis("off")
